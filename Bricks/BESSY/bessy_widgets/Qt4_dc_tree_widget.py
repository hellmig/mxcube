...
class DataCollectTree(QtGui.QWidget):
    ...
    def enable_collect(self, state):
        ...
        # 2016-06-10-bessy-mh-begin: functionality not yet tested, so disable buttons for time being
        # self.up_button.setDisabled(not state)
        # self.down_button.setDisabled(not state)
        self.up_button.setEnabled(False)
        self.down_button.setEnabled(False)
        # 2016-06-10-bessy-mh-end
                ...
                if not item.get_model().free_pin_mode:
                    if self.beamline_setup_hwobj.diffractometer_hwobj.in_plate_mode():
                        ...
                        if self.is_mounted_sample_item(item):
                            action = menu.addAction("Unmount", self.unmount_sample)
                        else:
                            action = menu.addAction("Mount", self.mount_sample)
                        if self.tree_brick.queue_execution_interlock_enabled:
        ...
        if len(items) == 1:
            ...
            if sample_changer:
                if hasattr(sample_changer, '__TYPE__')\
                   and sample_changer.__TYPE__ in ('CATS', 'Marvin', 'GROB_SC', 'ISARA2'):
