...
class XfeSpectrumBrick(BlissWidget):
    ...
    def run(self):
        print "************************************* XfeSpectrumBrick (BESSY version) ********************************"
        if self.xfeSpectrum is not None:
            if self.xfeSpectrum.isConnected():
                self.connected()
            else:
    ...
    def startXfeSpectrum(self):
        ...
        go_on=True
# 2014-01-28-bessy-mh: disable ISpyB hint
...
#                 go_on=False
        if self.sessionId is None or self.sessionId=="":
            logging.getLogger().warning("XRF Spectrum: No ISPyB login, your spectrum won't be stored in ISPyB")
        if not go_on:
...
# 2010-05-19-bessy-mh: begin - do not save to special archive but to normal data-collection directory
...
#             a_dir = os.path.join('/data/pyarch/',a[4],a[3],*a[5:])
        ...
        a_dir = str(self.directoryInput.text())
# 2010-05-19-bessy-mh: end
        ...
        # 2014-01-28-bessy-mh: do not copy data if archive dir not defined
