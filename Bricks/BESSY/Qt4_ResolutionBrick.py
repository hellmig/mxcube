...
class Qt4_ResolutionBrick(BlissWidget):
    ...
    def current_value_changed(self):
        ...
        if state == QtGui.QValidator.Acceptable: 
            ...
            if unit == chr(197):
                # new distance is given in Angstrom
                self.set_resolution(input_value)
            elif unit=="mm":
                self.set_detector_distance(input_value)
    ...
    def create_tool_tip(self):
        ...
        if self.detector_distance_limits:
            tool_tip = "Detector distance limits: %0.1f to %0.1f mm\n" \
                        %(self.detector_distance_limits[0], self.detector_distance_limits[1])
        if self.resolution_limits:
            unit = chr(197)
            tool_tip += "Resolution limits: %0.4f to %0.4f %s"\
    ...
    def update_gui(self, resolution_ready=None, detector_ready=None):
        ...
        if resolution_ready:
            ...
            self.get_resolution_limits(True, resolution_ready)
    ...
    def get_resolution_limits(self, force = False, resolution_ready = None):
        ...
        if resolution_ready:
            try:
              ...
              self.resolution_limits_changed(self.resolution_limits)
        else:
    ...
    def set_new_value_limits(self):
        ...
        if self.units_combobox.currentIndex() == 0:
            value_limits = self.resolution_limits
        else:
            value_limits = self.detector_distance_limits
        if value_limits is not None:
            if self.units_combobox.currentIndex() == 0:
                self.new_value_validator.setRange(value_limits[0], value_limits[1], 2)    
            else:
                # use 4 decimal places for the resolution
