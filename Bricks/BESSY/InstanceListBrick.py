...
class InstanceListBrick(BlissWidget):
    ...
    def propertyChanged(self, propertyName, oldValue, newValue):
        if propertyName == 'mnemonic':
            if self.instanceServer is not None:
                ...
                self.disconnect(self.instanceServer,PYSIGNAL('unauthorizedConnection'), self.handleUnauthorizedAccess)
            ...
            if self.instanceServer is not None:
                ...
                self.connect(self.instanceServer,PYSIGNAL('unauthorizedConnection'), self.handleUnauthorizedAccess)
    ...
    def hutchTriggerChanged(self, hutch_opened):
        if hutch_opened:
            if not BlissWidget.isInstanceUserIdInhouse():
              self.takeControlButton.setEnabled(True)
            elif not BlissWidget.isInstanceRoleServer():
    ...
    def handleUnauthorizedAccess(self):
        logging.info("Instance Server. Unauthorized accces") 
        if not BlissWidget.isInstanceUserIdInhouse():
