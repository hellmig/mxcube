...
class SampleChangerBrick3(BlissWidget):
    def __init__(self, *args):
        ...
        # 2014-01-15-bessy-mx: mh
        self.basketViews = []

        # new layout definition
        self.scView = QVBox(self)
        self.headerBox = QHBox(self.scView)
        self.headerBox.setSpacing(10)
        self.contentsBox = QHBox(self.scView)
        #self.contentsBox.setSpacing(10)
        self.footerBox = QHBox(self.scView)
        self.footerBox.setSpacing(10)
        QLabel("<b><i>Mount/unmount samples by right-clicking on the data collection tree on the left</b></i>", self.scView)
        VerticalSpacer(self.scView)
        #QLabel("<b>Dummy contents</b>", self.contentsBox)
        QVBoxLayout(self)
        self.layout().addWidget(self.scView)
      
        # layout status, current basket, current sample horizontally
        self.status=StatusView(self.headerBox)
        self.currentBasket=CurrentBasketView(self.headerBox)
        self.currentSample=CurrentSampleView(self.headerBox)
        # HorizontalSpacer(self.headerBox)

        # lay out buttons horizontally in footer box
        self.scanBaskets=ScanBasketsView(self.footerBox)
        self.cmdResetBasketsSamples = QPushButton("Reset SC contents", self.footerBox)
        QObject.connect(self.cmdResetBasketsSamples, SIGNAL("clicked()"), self.resetBasketsSamplesInfo)
        self.cmdSwitchToSampleTransfer = QPushButton("Sample Transfer mode", self.footerBox)
        self.doubleClickLoads=SCCheckBox("Double-click loads the sample",self.headerBox)
        self.doubleClickLoads.hide()
        # HorizontalSpacer(self.footerBox)
    ...
    def clearContentsBox(self):
        #self.contentsBox = QHBox(self.scView)
        #QLabel("<b>Dummy contents</b>", self.contentsBox)
        for curr_basket_view in self.basketViews:
            curr_basket_view.setParent(None)
            QObject.disconnect(curr_basket_view, PYSIGNAL("loadThisSample"), self.loadThisSample)
        self.contentsBox.update()


    def populateContentsBox(self, sampleChangerHO):
        #self.contentsBox = QHBox(self.scView)

        if sampleChangerHO is not None:
            no_of_baskets = len(sampleChangerHO.getComponents())

            if (no_of_baskets % 5) == 0:
	        # EMBL SC3 sample changer
	        no_of_containers = 1
	    else:
	        # CATS, tested for standard configuration with 9 baskets only
	        no_of_containers = 3 
            baskets_per_container = no_of_baskets / no_of_containers
             
            self.basketViews = []
            for j in range(no_of_containers):
                currentContainer = QVGroupBox("Container %d" % (j + 1), self.contentsBox)
              
                for i in range(baskets_per_container):
                    self.basketViews.append(BasketView(currentContainer, (j * baskets_per_container + i + 1)))
                    QObject.connect(self.basketViews[i],PYSIGNAL("loadThisSample"),self.loadThisSample)
                    self.basketViews[i].setChecked(False)
                    self.basketViews[i].setEnabled(False)
            self.contentsBox.update()
    ...
    def propertyChanged(self, propertyName, oldValue, newValue):
        if propertyName == 'icons':
            ...
            if self.sampleChanger is not None:
                self.clearContentsBox()
                self.disconnect(self.sampleChanger, SampleChanger.STATUS_CHANGED_EVENT, self.sampleChangerStatusChanged)
                self.disconnect(self.sampleChanger, SampleChanger.STATE_CHANGED_EVENT, self.sampleChangerStateChanged)
                self.disconnect(self.sampleChanger, SampleChanger.INFO_CHANGED_EVENT, self.infoChanged)
                self.disconnect(self.sampleChanger, SampleChanger.SELECTION_CHANGED_EVENT, self.selectionChanged)
                ...
                self.disconnect(self.sampleChanger, SampleChanger.LOADED_SAMPLE_CHANGED_EVENT, self.loadedSampleChanged)
            ...
            if self.sampleChanger is not None:
                ...
                # 2014-01-15-bessy-mx: mh - set up sample-changer view based on current sample changer HO
                self.populateContentsBox(self.sampleChanger)
            ...
            for basket in self.basketViews:
    ...
    def instanceModeChanged(self,mode):
        if mode==BlissWidget.INSTANCE_MODE_SLAVE:
            self.basketViewsSamplesSelectionDialog.reject()
    ...
    def sampleChangerContentsChanged(self, baskets):
        ...
        i=0
        for b in baskets:
          self.basketViews[i].blockSignals(True)
          self.basketViews[i].setChecked(b is not None)
          self.basketViews[i].blockSignals(False)
          i=i+1

    def scanBasket(self):
        if not self['showSelectButton']:
            self.basketViews[self.currentBasket.selected.value()-1].setChecked(True)
        self.sampleChanger.scan(self.sampleChanger.getSelectedComponent(), recursive=True, wait=False)

    def scanAllBaskets(self):
        baskets_to_scan = []
        for i, basket_checkbox in enumerate(self.basketViews):
    ...
    def infoChanged(self):
        ...
        for i, basket in enumerate(self.basketViews):
