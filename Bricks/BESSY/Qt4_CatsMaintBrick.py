...
from bessy_widgets.Qt4_catsmaintwidget import Ui_CatsMaintWidget
...
class Qt4_CatsMaintBrick(BlissFramework.Qt4_BaseComponents.BlissWidget):
    def __init__(self, *args):
        ...
        QtCore.QObject.connect(self.widget.btBackToolB, QtCore.SIGNAL('clicked()'), self._backToolBTraj)                     
    ...
    def _updateButtons(self):
        if self.device is None:
            ...
            self.widget.btBackToolB.setEnabled(False)
...
#             QtGui.QMessageBox.warning( self, "Error",str(sys.exc_info()[1]))
    ...
    def _backToolATraj(self):
        logging.getLogger("user_level_log").info("CATS: Transfer sample from tool A back to dewar.")
        try:
            if self.device is not None:
                self.device.backToolATraj()
        except:
            QtGui.QMessageBox.warning( self, "Error",str(sys.exc_info()[1]))

    def _backToolBTraj(self):
        logging.getLogger("user_level_log").info("CATS: Transfer sample from tool B back to dewar.")
        try:
            if self.device is not None:
                self.device.backToolBTraj()
        except:
            QtGui.QMessageBox.warning( self, "Error",str(sys.exc_info()[1]))

    def _dryToolTraj(self):
        logging.getLogger("user_level_log").info("CATS: Dry the gripper tool.")
        try:
            if self.device is not None:
    ...
    def _soakToolTraj(self):
        logging.getLogger("user_level_log").info("CATS: Cool down (soak) the gripper tool in technical port (Lid #2).")
        try:
            if self.device is not None:
